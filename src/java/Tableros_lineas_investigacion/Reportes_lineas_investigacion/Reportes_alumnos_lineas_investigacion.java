...
package Tableros_lineas_investigacion.Reportes_lineas_investigacion;
...
public class Reportes_alumnos_lineas_investigacion extends HttpServlet {
    ...
    String correo[]=new String[conteoReg];
    String nivel[]=new String[conteoReg];
    String tipo[]=new String[conteoReg];
    String status[]=new String[conteoReg];
    
    try 
    {      
    for(int i=0; i<conteoReg; i++){
    nombre[i]=new String(request.getParameter("nombre"+i).getBytes("iso-8859-1"),"UTF-8");
    correo[i]=new String(request.getParameter("correo"+i).getBytes("iso-8859-1"),"UTF-8");
    nivel[i]=new String(request.getParameter("nivel"+i).getBytes("iso-8859-1"),"UTF-8");
    tipo[i]=new String(request.getParameter("tipo"+i).getBytes("iso-8859-1"),"UTF-8");
    status[i]=new String(request.getParameter("status"+i).getBytes("iso-8859-1"),"UTF-8");
    }
    ...
    Paragraph titulo_doc = new Paragraph("Registro de alumnos",tnr1);
    ...
    for(int i=0; i<conteoReg2; i++){
        ...
        for(int x=0; x<conteoNum[i]; x++){
         ...
         tabla.addCell(correo[y]); 
         tabla.addCell(nivel[y]);
         tabla.addCell(tipo[y]); 
          tabla.addCell(status[y]); 
         y++;
