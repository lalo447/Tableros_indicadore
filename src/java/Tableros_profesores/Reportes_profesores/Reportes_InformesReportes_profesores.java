...
public class Reportes_InformesReportes_profesores extends HttpServlet {
    ...
    String serie_documento[]=new String[conteoReg];
    String num_volumen[]=new String[conteoReg];
    String rango_paginas[]=new String[conteoReg];
    String fecha_bus_con[]=new String[conteoReg];
    
    try 
    {      
    for(int i=0; i<conteoReg; i++){
    titulo[i]=new String(request.getParameter("titulo"+i).getBytes("iso-8859-1"),"UTF-8");
    serie_documento[i]=new String(request.getParameter("serie"+i).getBytes("iso-8859-1"),"UTF-8");
    num_volumen[i]=new String(request.getParameter("volumen"+i).getBytes("iso-8859-1"),"UTF-8");
    rango_paginas[i]=new String(request.getParameter("rango_p"+i).getBytes("iso-8859-1"),"UTF-8");
    fecha_bus_con[i]=new String(request.getParameter("fecha_bus_con"+i).getBytes("iso-8859-1"),"UTF-8");
    }
    ...
    Paragraph titulo_doc = new Paragraph("Registro de Informes",tnr1);
    ...
    for(int i=0; i<conteoReg2; i++){
        ...
        for(int x=0; x<conteoNum[i]; x++){
         ...
         tabla.addCell(serie_documento[y]); 
         tabla.addCell(num_volumen[y]);
         tabla.addCell(rango_paginas[y]); 
          tabla.addCell(fecha_bus_con[y]); 
         y++;
