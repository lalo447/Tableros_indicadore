...
public class Reportes_proyectos_profesores extends HttpServlet {
    ...
    String fecha_inicio[]=new String[conteoReg];
    String fecha_final[]=new String[conteoReg];
    String estado[]=new String[conteoReg];
    String tipo_actividad[]=new String[conteoReg];
    
     try 
    { 
    for(int i=0; i<conteoReg; i++){
    titulo[i]=new String(request.getParameter("titulo"+i).getBytes("iso-8859-1"),"UTF-8");
    fecha_inicio[i]=new String(request.getParameter("fecha_inicial"+i).getBytes("iso-8859-1"),"UTF-8");
    fecha_final[i]=new String(request.getParameter("fecha_bus_con"+i).getBytes("iso-8859-1"),"UTF-8");
    estado[i]=new String(request.getParameter("estado"+i).getBytes("iso-8859-1"),"UTF-8");
    tipo_actividad[i]=new String(request.getParameter("tipo_actividad"+i).getBytes("iso-8859-1"),"UTF-8");
    }
    ...
    Paragraph titulo_doc = new Paragraph("Registro de proyectos",tnr1);
    ...
    for(int i=0; i<conteoReg2; i++){
        ...
        Paragraph t2=new Paragraph("Fecha inicial",tnr2);
        PdfPCell c2 = new PdfPCell(new Phrase(t2));
        c2.setBackgroundColor(celdaColor);
        tabla.addCell(c2);
        
        Paragraph t3=new Paragraph("Fecha final",tnr2);
        ...
        Paragraph t5=new Paragraph("Tipo de actividad",tnr2);
        ...
        for(int x=0; x<conteoNum[i]; x++){
         ...
         tabla.addCell(fecha_inicio[y]); 
         tabla.addCell(fecha_final[y]); 
         tabla.addCell(estado[y]); 
         tabla.addCell(tipo_actividad[y]);
         y++;
