<%-- 
    Document   : Tableros_derechos_autor_alumnos
    Created on : 14 may. 2020, 8:29:51
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-user-graduate"></i> Derechos autor de alumnos</h2>
        ...
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.numero_tramite, p.fch_registro, p.fch_solicitud from usuario_alumno u, alm_colbr_dcho pr, patente_dcho_autor p  where u.numero_control=pr.numero_control and pr.clave_dcho_autor=p.clave_dcho_autor order by u.nombre");
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           stNumRegistros=con.createStatement();    
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_dcho pr, patente_dcho_autor p  where u.numero_control=pr.numero_control and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' group by u.numero_control order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_dcho pr, patente_dcho_autor p  where u.numero_control=pr.numero_control and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.numero_tramite, p.fch_registro, p.fch_solicitud from usuario_alumno u, alm_colbr_dcho pr, patente_dcho_autor p  where u.numero_control=pr.numero_control and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' order by u.nombre");
           fechasBusqueda="true";
            }            
           }
        ...
        <form method="POST" action="./Reportes_derechos_autor_alumnos">
