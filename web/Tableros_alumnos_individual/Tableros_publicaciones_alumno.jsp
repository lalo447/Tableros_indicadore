<%-- 
    Document   : Tableros_publicaciones_alumno
    Created on : 14 may. 2020, 9:37:20
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-user-graduate"></i> Publicaciones de alumnos</h2>
       ...
       /*Realizamos los query de consultas de publicaciones*/
        ...
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and u.numero_control='"+numSelect+"' group by u.numero_control order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and u.numero_control='"+numSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_publicacion, p.tipo_publicacion, p.ano_publicacion, p.estado_publicacion, p.nombre_publicador, p.num_volumen, p.resumen, p.link_pub from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and u.numero_control='"+numSelect+"' order by u.nombre");
        ...
        if(f2>=f1){
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' group by u.numero_control order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_publicacion, p.tipo_publicacion, p.ano_publicacion, p.estado_publicacion, p.nombre_publicador, p.num_volumen, p.resumen, p.link_pub from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' order by u.nombre");
        fechasBusqueda="true";  
        }
        ...
        <form method="POST" action="./Reportes_publicaciones_alumnos">
        ...
        <input type="hidden" name="asignaciones" value="Asignaciones totales de publicaciones">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
      ...
      <td><a href="./Resumen_tableros_alumno?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de la publicacion"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
...
<script>
 ...
 alert("Error la fecha inicial no puede ser mayor a la final");
 }  
 }
</script>     
