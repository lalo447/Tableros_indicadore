<%-- 
    Document   : Tableros_profesores_lineas_investigacion
    Created on : 16 may. 2020, 11:16:26
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
       ...
       /*Realizamos los query de consultas de los profesores*/
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select l.nombre_linea, count(u.rfc) from usuario_profesor u, linea_investigacion l  where u.clave_linea=l.clave_linea group by l.nombre_linea order by l.nombre_linea");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, linea_investigacion l where u.clave_linea=l.clave_linea");      
        st=con.createStatement();
        rs=st.executeQuery("select l.nombre_linea, u.nombre, u.apellido_p, u.apellido_m, u.correo, u.tipo_profesor, u.status_profesor from linea_investigacion l, usuario_profesor u where l.clave_linea=u.clave_linea order by l.nombre_linea");
        ...
        <input type="hidden" name="titulo" value="Graficas de profesores">
        <input type="hidden" name="tituloG" value="Profesores asignados">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
