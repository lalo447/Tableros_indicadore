<%-- 
    Document   : Tableros_departamentos_institucion
    Created on : 27 may. 2020, 10:11:51
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        String InstNombres[]=(String [])request.getAttribute("InstNombresDep");
        String nombre[]=(String [])request.getAttribute("nombreDep");
        String jefe[]=(String [])request.getAttribute("jefeDep");
       
        String nombresDep[]=(String [])request.getAttribute("nombresDep");
        int numDep[]=(int [])request.getAttribute("numDep");
        String opcBordeDep[]=(String [])request.getAttribute("opcBordeDep"); 
        String nombresCuerpo[]=(String [])request.getAttribute("nombresCurso");
        ...
        <input type="hidden" name="conteoReg" id="conteoReg" value="<%=conteoRegDep%>"> 
        <input type="hidden" name="conteoReg2" id="conteoReg2" value="<%=conteoRegDep2%>">
        <%for(int i=0; i<conteoRegDep2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombresDep[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numDep[i]%>">
        <%}%>
        <%for(int i=0; i<conteoRegDep; i++){%>
      ...
      <tbody>
      <%for(int i=0; i<conteoRegDep; i++){%>
      <tr class="<%=opcBordeDep[i]%>">
      <td class="text-left"><%=InstNombres[i]%></td>
