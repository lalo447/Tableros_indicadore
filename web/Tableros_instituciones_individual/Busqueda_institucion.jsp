<%-- 
    Document   : Busqueda_institucion
    Created on : 26 may. 2020, 13:01:46
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-school"></i> Busqueda de institucion</h2>
        ...
        clave_institucion=(String [])request.getAttribute("clave_institucion");
        nombre=(String [])request.getAttribute("nombre");
        conteoReg=clave_institucion.length;
        }

        else if(campoBusqueda!=null){ 
        clave_institucion=null;
        nombre=null;
         stConteo=con.createStatement();
         rsConteo=stConteo.executeQuery("select count(*) from institucion where clave_institucion='"+campoBusqueda+"' OR nombre_institucion='"+campoBusqueda+"'");
         st=con.createStatement();
         rs=st.executeQuery("select clave_institucion, nombre_institucion from institucion where clave_institucion='"+campoBusqueda+"' OR nombre_institucion='"+campoBusqueda+"' order by nombre_institucion");             
        ...
        clave_institucion=new String[conteoReg];
       ...
       <tr>
       <th scope="col" class="text-left">Clave de institucion</th>
      ...
      <tr class="">
      <td class="text-left"><%=clave_institucion[i]%></td>   
      <td class="text-left"><%=nombre[i]%></td>
      <td class="text-left"><a href="./Tableros_institucion?clave=<%=clave_institucion[i]%>&nombreC=<%=nombre[i]%>"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Ver tableros"><i class="fas fa-paste"></i></button></a></td>
      </tr>
      <%}%>
      </tbody>
      </table>
