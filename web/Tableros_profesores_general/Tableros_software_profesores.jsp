<%-- 
    Document   : Tableros_software_profesores
    Created on : 7 may. 2020, 9:00:15
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.horas_hombre, p.pais_desarrolla, p.fch_terminacion, p.resumen from usuario_profesor u, prof_colbr_soft pr, patente_software p  where u.rfc=pr.rfc and pr.clave_patente_soft=p.clave_patente_soft order by u.nombre");
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           stNumRegistros=con.createStatement();
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_soft pr, patente_software p  where u.rfc=pr.rfc and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_soft pr, patente_software p  where u.rfc=pr.rfc and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.horas_hombre, p.pais_desarrolla, p.fch_terminacion, p.resumen from usuario_profesor u, prof_colbr_soft pr, patente_software p  where u.rfc=pr.rfc and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' order by u.nombre");
           fechasBusqueda="true";
            }            
           }
      ...
      <td class="text-left"><%=fecha_bus_con[i]%></td>
       <td><a href="./Resumen_tableros_profesores?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen del software"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
       </tr>
      <%}%>
      </tbody>
      </table>
