<%-- 
    Document   : Tableros_informes_reportes_profesores
    Created on : 6 may. 2020, 14:48:14
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-chalkboard-teacher"></i> Informes o reportes de profesores</h2>
        ...
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_informe, p.serie_documento, p.num_volumen, p.rango_paginas, p.fch_publicado from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep order by u.nombre");
         
        
         /*Realizamos los query de consultas por busqueda por fecha de informes*/
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           stNumRegistros=con.createStatement();
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_informe, p.serie_documento, p.num_volumen, p.rango_paginas, p.fch_publicado from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' order by u.nombre");
           fechasBusqueda="true";
           }            
           }
        ...
        asi mismo obtenemos en numero de informes por cada profesor de acuerdo a la consulta
        ...
        <form method="POST" action="./Reportes_InformesReportes_profesores">
        ...
        <input type="hidden" name="fecha_hasta" value="<%=fecha_hasta%>">
        <input type="hidden" name="fechas_busqueda" value="<%=fechasBusqueda%>">
        <input type="hidden" name="titulo" value="Graficas de informes reportes">
        <input type="hidden" name="tituloG" value="Informes asignados">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
