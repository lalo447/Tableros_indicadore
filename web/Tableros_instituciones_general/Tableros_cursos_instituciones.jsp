<%-- 
    Document   : Tableros_cursos_instituciones
    Created on : 26 may. 2020, 11:47:42
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
       ...
       /*Realizamos los query de consultas de los cursos*/
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select i.nombre_institucion, count(clave_curso) from cursos c, institucion i where c.clave_institucion=i.clave_institucion group by i.nombre_institucion order by i.nombre_institucion");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from cursos c, institucion i where c.clave_institucion=i.clave_institucion order by i.nombre_institucion");      
        st=con.createStatement();
        rs=st.executeQuery("select i.nombre_institucion, c.nombre_curso, c.tipo_curso, c.numero_horas, c.fecha from cursos c, institucion i where c.clave_institucion=i.clave_institucion order by i.nombre_institucion");
        ...
        <input type="hidden" name="titulo" value="Graficas de cursos">
        <input type="hidden" name="tituloG" value="Cuerpos academicos asignados">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
