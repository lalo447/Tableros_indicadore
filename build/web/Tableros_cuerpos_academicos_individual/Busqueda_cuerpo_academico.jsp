<%-- 
    Document   : Busqueda_cuerpo_academico
    Created on : 22 may. 2020, 9:27:42
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-archive"></i> Busqueda de cuerpos academicos</h2>
        ...
        String clave_cuerpo[]=null;
        String nombre_cuerpo[]=null;
        String jefe[]=null;
        String nombre_linea[]=null;
        ...
        clave_cuerpo=(String [])request.getAttribute("clave_cuerpo");    
        nombre_cuerpo=(String [])request.getAttribute("nombre_cuerpo");
        jefe=(String [])request.getAttribute("jefe");
        nombre_linea=(String [])request.getAttribute("nombre_linea");
        nombre_red=(String [])request.getAttribute("nombre_red");
        conteoReg=nombre_cuerpo.length;
        }
        ...
        nombre_linea=null;
         ...
         rsConteo=stConteo.executeQuery("select count(*) from cuerpo_academico where clave_cuerpo='"+campoBusqueda+"' or nombre_cuerpo='"+campoBusqueda+"'");
         st=con.createStatement();
         rs=st.executeQuery("select clave_cuerpo, nombre_cuerpo, jefe_cuerpo, clave_linea, (select nombre_red from red_colaboracion where clave_red=c.clave_red) from cuerpo_academico c where clave_cuerpo='"+campoBusqueda+"' or nombre_cuerpo='"+campoBusqueda+"'");             
        ...
        clave_cuerpo=new String[conteoReg];
        nombre_cuerpo=new String[conteoReg];
        jefe=new String[conteoReg];
        nombre_linea=new String[conteoReg];
        nombre_red=new String[conteoReg];
        
        if(conteoReg>0){
        int x=0;
        while(rs.next()){
            clave_cuerpo[x]=rs.getString(1);
            nombre_cuerpo[x]=rs.getString(2);
            jefe[x]=rs.getString(3);
            nombre_linea[x]=rs.getString(4);
            nombre_red[x]=rs.getString(5);
            x++;
        }
         rs.beforeFirst();
       ...
       <tr>
       <th scope="col" class="text-left">Clave de cuerpo</th>
      ...
      <tr class="">
      <td class="text-left"><%=clave_cuerpo[i]%></td>   
      <td class="text-left"><%=nombre_cuerpo[i]%></td>
      <td class="text-left"><%=nombre_linea[i]%></td>
      <td class="text-left"><%=jefe[i]%></td>
      <td class="text-left"><%=nombre_red[i]%></td>
      <td class="text-left"><a href="./Tableros_cuerpo_academico?clave=<%=clave_cuerpo[i]%>&nombreC=<%=nombre_cuerpo[i]%>"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Ver tableros"><i class="fas fa-paste"></i></button></a></td>
      </tr>
      <%}%>
      </tbody>
      </table>
