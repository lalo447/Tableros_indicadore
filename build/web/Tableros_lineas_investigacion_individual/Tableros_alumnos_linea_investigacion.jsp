<%-- 
    Document   : Tableros_linea_registros
    Created on : 17 may. 2020, 10:57:08
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        String LineaNombresAlm[]=(String [])request.getAttribute("LineaNombresAlm");
        String nombreAlm[]=(String [])request.getAttribute("nombreAlm");
        String correoAlm[]=(String [])request.getAttribute("correoAlm");
        String nivelAlm[]=(String [])request.getAttribute("nivelAlm");
        String tipoAlm[]=(String [])request.getAttribute("tipoAlm");
        String statusAlm[]=(String [])request.getAttribute("statusAlm");
        String nombresAlm[]=(String [])request.getAttribute("nombresAlm");
        int numAlm[]=(int [])request.getAttribute("numAlm");
        String opcBordeAlm[]=(String [])request.getAttribute("opcBordeAlm"); 
     ...
     <div class="form-inline row my-2">
        ...
        <input type="hidden" name="conteoReg" id="conteoReg" value="<%=conteoRegAlm%>"> 
        <input type="hidden" name="conteoReg2" id="conteoReg2" value="<%=conteoRegAlm2%>">
        <input type="hidden" name="conteoTotal" value="<%=conteoTotal%>">
        <%for(int i=0; i<conteoRegAlm2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombresAlm[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numAlm[i]%>">
        <%}%>
        <%for(int i=0; i<conteoRegAlm; i++){%>
        <input type="hidden" name="nombre<%=i%>" value="<%=nombreAlm[i]%>">
        <input type="hidden" name="correo<%=i%>" value="<%=correoAlm[i]%>">
        <input type="hidden" name="nivel<%=i%>" value="<%=nivelAlm[i]%>">
        <input type="hidden" name="tipo<%=i%>" value="<%=tipoAlm[i]%>">
        <input type="hidden" name="status<%=i%>" value="<%=statusAlm[i]%>">
        <%}%>
      ...
      <tbody>
      <%for(int i=0; i<nombreAlm.length; i++){%>
      <tr class="<%=opcBordeAlm[i]%>">
      <td class="text-left"><%=LineaNombresAlm[i]%></td>
      <td class="text-left"><%=nombreAlm[i]%></td>
      <td class="text-left"><%=correoAlm[i]%></td>
      <td class="text-left"><%=nivelAlm[i]%></td>
      <td class="text-left"><%=tipoAlm[i]%></td>
      <td class="text-left"><%=statusAlm[i]%></td>
     </tr>
      <%}%>
      </tbody>
      </table>
      ...
      <div class="form-inline justify-content-between">
        <div class="text-muted mb-2">Total de asignaciones: <input type="text" class="text-center" value="<%=conteoRegAlm%>" size=2 readonly></div> 
