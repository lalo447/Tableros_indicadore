<%-- 
    Document   : Tableros_derechos_autor_profesores
    Created on : 7 may. 2020, 10:25:34
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-chalkboard-teacher"></i> Derechos autor de profesores</h2>
        ...
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.numero_tramite, p.fch_registro, p.fch_solicitud from usuario_profesor u, prof_colbr_dcho pr, patente_dcho_autor p  where u.rfc=pr.rfc and pr.clave_dcho_autor=p.clave_dcho_autor order by u.nombre");
       
         /*Realizamos los query de consultas por busqueda por fecha de derechos*/
        if(fecha_desde!=null && fecha_hasta!=null){ 
            Date date1 = sdf.parse(fecha_desde); 
            Date date2 = sdf.parse(fecha_hasta); 
            
           if(date2.compareTo(date1)==1){
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_dcho pr, patente_dcho_autor p  where u.rfc=pr.rfc and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_dcho pr, patente_dcho_autor p  where u.rfc=pr.rfc and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.numero_tramite, p.fch_registro, p.fch_solicitud from usuario_profesor u, prof_colbr_dcho pr, patente_dcho_autor p  where u.rfc=pr.rfc and pr.clave_dcho_autor=p.clave_dcho_autor and fch_solicitud>='"+fecha_desde+"' and fch_solicitud<='"+fecha_hasta+"' order by u.nombre");
           fechasBusqueda="true";
            }            
           }
        ...
        num_tramite=new String[conteoReg];
        fecha_registro=new String[conteoReg];
        ...
        asi mismo obtenemos en numero de derechos por cada profesor de acuerdo a la consulta
        ...
        while(rs.next()){
            ...
            num_tramite[x]=rs.getString(7);
            fecha_registro[x]=rs.getString(8);
        ...
        <form method="POST" action="./Reportes_derechos_autor_profesores">
        ...
        <input type="hidden" name="num_tramite<%=i%>" value="<%=num_tramite[i]%>">
        <input type="hidden" name="fecha_registro<%=i%>" value="<%=fecha_registro[i]%>">
        ...
        <input type="hidden" name="fecha_hasta" value="<%=fecha_hasta%>">
        <input type="hidden" name="fechas_busqueda" value="<%=fechasBusqueda%>">
        <input type="hidden" name="titulo" value="Graficas de derechos autor">
       ...
       <th scope="col" class="text-left">Tramite</th>
       <th scope="col" class="text-left">Fecha de registro</th>
       <th scope="col" class="text-left">Fecha de solicitud</th>
       </tr>
       </thead>
      ...
      <td class="text-left"><%=num_tramite[i]%></td>
      <td class="text-left"><%=fecha_registro[i]%></td>
