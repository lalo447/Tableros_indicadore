<%-- 
    Document   : Tableros_patentes_profesores
    Created on : 7 may. 2020, 9:29:01
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-chalkboard-teacher"></i> Patentes de profesores</h2>
        ...
        String tipo[]=null;
        String pais[]=null;
        String fecha_presentacion[]=null;
        ...
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.tipo_patente, p.pais, p.fch_presentacion, p.fch_concesion, p.resumen from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente order by u.nombre");
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.tipo_patente, p.pais, p.fch_presentacion, p.fch_concesion, p.resumen from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' order by u.nombre");
           fechasBusqueda="true";
           }            
           }
        ...
        fecha_presentacion=new String[conteoReg];
        ...
        while(rs.next()){
            ...
            fecha_presentacion[x]=rs.getString(9);
            fecha_bus_con[x]=rs.getString(10);
        ...
        <form method="POST" action="./Reportes_patentes_profesores">
        ...
        <input type="hidden" name="fecha_presentacion<%=i%>" value="<%=fecha_presentacion[i]%>">
        ...
        <input type="hidden" name="fecha_hasta" value="<%=fecha_hasta%>">
        <input type="hidden" name="fechas_busqueda" value="<%=fechasBusqueda%>">
        <input type="hidden" name="titulo" value="Graficas de patentes">
        <input type="hidden" name="tituloG" value="Patentes asignadas">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
       ...
       <th scope="col" class="text-left">Fecha de presentacion</th>
       <th scope="col" class="text-left">Fecha de consecion</th>
      ...
      <td class="text-left"><%=fecha_presentacion[i]%></td>
      <td class="text-left"><%=fecha_bus_con[i]%></td>
       <td><a href="./Resumen_tableros_profesores?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de proyecto de patente"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
       </tr>
      <%}%>
      </tbody>
      </table>
