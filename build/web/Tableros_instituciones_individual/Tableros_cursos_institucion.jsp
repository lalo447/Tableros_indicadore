<%-- 
    Document   : Tableros_cursos_institucion
    Created on : 27 may. 2020, 10:12:36
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        <h2 class="text-muted"><i class="fas fa-school"></i> Cursos de instituciones</h2>
        ...
        int conteoRegCurso=(Integer)request.getAttribute("conteoRegCurso");
        int conteoRegCurso2=(Integer)request.getAttribute("conteoRegCurso2");
        
        String InstNombresCurso[]=(String [])request.getAttribute("InstNombresCurso");
        String nombre[]=(String [])request.getAttribute("nombreCurso");
        String numero_horas[]=(String [])request.getAttribute("numero_horasCurso");
        String tipo[]=(String [])request.getAttribute("tipoCurso");
        String fecha[]=(String [])request.getAttribute("fechaCurso");
        
        String opcBordeCurso[]=(String [])request.getAttribute("opcBordeCurso"); 
        String nombresDep[]=(String [])request.getAttribute("nombresDep");
        int numDep[]=(int [])request.getAttribute("numDep");
        String nombresCurso[]=(String [])request.getAttribute("nombresCurso");
        int numCurso[]=(int [])request.getAttribute("numCurso");
        
        int numRegistros[]=new int[2];
        numRegistros[0]=numDep[0];
        numRegistros[1]=numCurso[0];
        ...
        int conteoTotal=(conteoRegDep+conteoRegCurso);    
        ...
        <form method="POST" action="./Reportes_cursos_instituciones">
        <input type="hidden" name="conteoReg" id="conteoReg" value="<%=conteoRegCurso%>"> 
        <input type="hidden" name="conteoReg2" id="conteoReg2" value="<%=conteoRegCurso2%>">
        <%for(int i=0; i<conteoRegCurso2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombresCurso[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numCurso[i]%>">
        <%}%>
        <%for(int i=0; i<conteoRegCurso; i++){%>
        <input type="hidden" name="nombre<%=i%>" value="<%=nombre[i]%>">
        <input type="hidden" name="tipo<%=i%>" value="<%=tipo[i]%>">
        <input type="hidden" name="numero_horas<%=i%>" value="<%=numero_horas[i]%>">
        <input type="hidden" name="fecha<%=i%>" value="<%=fecha[i]%>">
        <%}%>
        ...
        <form method="POST" action="./Graficas_de_tableros_institucion">
        ...
        <input type="hidden" name="registro" id="registro" value="<%=(String)request.getAttribute("nombreC")%>">
       ...
       <th scope="col" class="text-left">Horas</th>
       <th scope="col" class="text-left">Fecha</th>
       </tr>
       </thead>
     <tbody>
      <%for(int i=0; i<conteoRegCurso; i++){%>
      <tr class="<%=opcBordeCurso[i]%>">
      <td class="text-left"><%=InstNombresCurso[i]%></td>
      <td class="text-left"><%=nombre[i]%></td>
      <td class="text-left"><%=tipo[i]%></td>
      <td class="text-left"><%=numero_horas[i]%></td>
      <td class="text-left"><%=fecha[i]%></td>
     </tr>
      <%}%>
      </tbody>
      </table>
