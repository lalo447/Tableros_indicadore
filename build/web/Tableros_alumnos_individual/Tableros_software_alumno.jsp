<%-- 
    Document   : Tableros_software_alumno
    Created on : 14 may. 2020, 10:53:41
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        <h2 class="text-muted"><i class="fas fa-user-graduate"></i> Software de alumnos</h2>
        <input type="hidden" name="reseteo" value="yes">
        <input type="hidden" name="num" value="<%=numSelect%>"> 
        <button type="submit" class="btn busqueda" style="background:#5D96F5; color: #fff;" title="Reiniciar registras">
        ...
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and u.numero_control='"+numSelect+"' group by u.numero_control order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and u.numero_control='"+numSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.horas_hombre, p.pais_desarrolla, p.fch_terminacion, p.resumen from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and u.numero_control='"+numSelect+"' order by u.nombre");
       
         /*Realizamos los query de consultas por busqueda por fecha de software*/
        if(fecha_desde!=null && fecha_hasta!=null){ 
          ...
          stNumRegistros=con.createStatement();
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' group by u.numero_control order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.horas_hombre, p.pais_desarrolla, p.fch_terminacion, p.resumen from usuario_alumno u, alm_colbr_soft pr, patente_software p  where u.numero_control=pr.numero_control and pr.clave_patente_soft=p.clave_patente_soft and fch_terminacion>='"+fecha_desde+"' and fch_terminacion<='"+fecha_hasta+"' and u.numero_control='"+numSelect+"' order by u.nombre");
           fechasBusqueda="true";
            }            
           }
        ...
        horas=new String[conteoReg];
        pais=new String[conteoReg];
        ...
        asi mismo obtenemos en numero de software por cada profesor de acuerdo a la consulta
        ...
        while(rs.next()){
            ...
            estado[x]=rs.getString(6);
            horas[x]=rs.getString(7);
            pais[x]=rs.getString(8);
        ...
        <form method="POST" action="./Reportes_software_alumnos">
        ...
        <input type="hidden" name="horas<%=i%>" value="<%=horas[i]%>">
        <input type="hidden" name="pais<%=i%>" value="<%=pais[i]%>">
        ...
        <input type="hidden" name="fecha_hasta" value="<%=fecha_hasta%>">
        <input type="hidden" name="fechas_busqueda" value="<%=fechasBusqueda%>">
        <input type="hidden" name="titulo" value="Graficas de software">
        <input type="hidden" name="tituloG" value="Software asignados">
        <input type="hidden" name="asignaciones" value="Asignaciones totales de software">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
       ...
       <th scope="col" class="text-left">Horas de desarrollo</th>
       <th scope="col" class="text-left">Pais</th>
       <th scope="col" class="text-left">Fecha de terminacion</th>
      ...
      <td class="text-left"><%=horas[i]%></td>
      <td class="text-left"><%=pais[i]%></td>
      <td class="text-left"><%=fecha_bus_con[i]%></td>
       <td><a href="./Resumen_tableros_alumno?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen del software"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
       </tr>
      <%}%>
      </tbody>
      </table>
