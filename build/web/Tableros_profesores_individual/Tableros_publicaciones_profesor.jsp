<%-- 
    Document   : Tableros_publicacion_profesor
    Created on : 9 may. 2020, 9:09:23
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_pub pr, publicaciones p  where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_pub pr, publicaciones p  where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and u.rfc='"+rfcSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_publicacion, p.tipo_publicacion, p.ano_publicacion, p.estado_publicacion, p.nombre_publicador, p.num_volumen, p.resumen, p.link_pub from usuario_profesor u, prof_colbr_pub pr, publicaciones p where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and u.rfc='"+rfcSelect+"' order by u.nombre");
        ...
        if(f2>=f1){
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_pub pr, publicaciones p  where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_pub pr, publicaciones p  where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_publicacion, p.tipo_publicacion, p.ano_publicacion, p.estado_publicacion, p.nombre_publicador, p.num_volumen, p.resumen, p.link_pub from usuario_profesor u, prof_colbr_pub pr, publicaciones p  where u.rfc=pr.rfc and pr.clave_publicaion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");
        fechasBusqueda="true";  
        }
        }
      ...
      <td><a href="./Resumen_tableros_profesor?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de la publicacion"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
