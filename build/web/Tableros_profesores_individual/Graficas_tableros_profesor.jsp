<%-- 
    Document   : Graficas_tableros_profesor
    Created on : 8 may. 2020, 12:09:44
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
        ...
        String asignaciones=(String)request.getAttribute("asignaciones");
        
        /*Consulta general*/
        String nombresRG[]=(String [])request.getAttribute("nombresRG");
        int fechas_bus_recRG[]=(int [])request.getAttribute("fechas_bus_recRG");
        int numFechasRG[]=(int [])request.getAttribute("numFechasRG");
        int conteoRG=(Integer)request.getAttribute("conteoConsulta");
        %>
         
	<div class="main_container">
	<div class="container">
	<div class="content" style="z-index: 2; height: 110vh; min-width: 650px;">
        <h3 class="text-muted"><i class="fas fa-chalkboard-teacher"></i> <%=titulo%></h3>
        ...
        <!-- Enviamos los datos para la consulta general-->
         <%for(int i=0; i<nombresRG.length; i++){%>
        <input type="hidden" id="nombresRG<%=i%>" value="<%=nombresRG[i]%>">
        <%}%>
         <%for(int i=0; i<fechas_bus_recRG.length; i++){%>
        <input type="hidden" id="fechas_bus_recRG<%=i%>" value="<%=fechas_bus_recRG[i]%>">
        <%}%>
         <%for(int i=0; i<numFechasRG.length; i++){%>
        <input type="hidden" id="numFechasRG<%=i%>" value="<%=numFechasRG[i]%>">
        <%}%>       
        <input type="hidden" id="conteo_nombresRG" value="<%=nombresRG.length%>">
        <input type="hidden" id="conteo_fechas_bus_recRG" value="<%=fechas_bus_recRG.length%>">
        <input type="hidden" id="conteo_numFechasRG" value="<%=numFechasRG.length%>">
        
        
        <div class="form-group">
        <label class="text-muted mr-2">Selecciona un tipo de grafica</label>
       <select name="select" class="form-control" onchange="Graficas(this.value)" style="background: #30B5A8; color: #fff;">
       ...
       <option value="Linea">Linea</option>
        ...
        <div class="form-inline">
          <p class="text-muted my-2"><%=nombres[0]%> <%=titulog.toLowerCase() %> <%=(Integer)request.getAttribute("conteoReg")%>, <%=asignaciones.toLowerCase()%> <%=conteoRG%></p>
          <%if(!fechas_busqueda.equals("")){%>
          <p class="text-muted my-2">, rango de <%=fecha_desde%> hasta <%=fecha_hasta%></p>
          <%}%>
        </div>
       
         <div class="form-inline">
        <label id="radio-group1" for="radioChecked1" class="radio text-muted mx-2">
        <input id="radioChecked1" type="radio" name="radio" value="Individual" onchange="opcionConsulta(this.value)">
        Individual
        <span></span>
        </label>  
         
        <label id="radio-group2" for="radioChecked2" class="radio text-muted mx-2">
        <input id="radioChecked2" type="radio" name="radio" value="General" onchange="opcionConsulta(this.value)">
        General
        <span></span>
        </label>
        </div>
 ...
 <script>
     ...
     document.getElementById('radio-group2').style.display="none";
      ...
      function Graficas(v){
        ...
        var graphOutlines = [];
        var hoverColor = [];
        var internalDataLength = conteo_fechas_rec;
        i = 0;
        ...
        return ""+tituloG+": "+ tooltipItems.yLabel;
        }
        } 
        }
        }
	...
	animateScale: true
        ...
        var fechas_rec2=new Array(conteo_fechas_rec+1);
        var num_fechas2=new Array(conteo_num_fechas+1);
        
        num_fechas2[0]=0;
        fechas_rec2[0]='Valor minimo';
        var x=0;
        
        for(var i=1; i<=conteo_fechas_rec; i++){
        fechas_rec2[i]=document.getElementById('fechas_bus_rec'+x).value;
        x++;
        }
        x=0;
        
        for(var i=1; i<=conteo_fechas_rec; i++){  
        num_fechas2[i]=document.getElementById('numFechas'+x).value;
        x++;
        }
        x=0;
      ...
      if(v=='Individual'){   
       ...
       var conteo_nom_RG=document.getElementById('conteo_nombresRG').value;
       var conteo_fechas_rec_RG=document.getElementById('conteo_fechas_bus_recRG').value;
       var conteo_num_fechas_RG=document.getElementById('conteo_numFechasRG').value;
     
       var nom_RG=new Array(conteo_nom_RG);
       var fechas_rec_RG=new Array(conteo_fechas_rec_RG);
       var num_fechas_RG=new Array(conteo_num_fechas_RG);
       
       for(i=0; i<conteo_nom_RG; i++){
          nom_RG[i]=document.getElementById('nombresRG'+i).value; 
       }
       
       for(i=0; i<conteo_fechas_rec_RG; i++){
          fechas_rec_RG[i]=document.getElementById('fechas_bus_recRG'+i).value; 
       }
       
        for(i=0; i<conteo_num_fechas_RG; i++){
          num_fechas_RG[i]=document.getElementById('numFechasRG'+i).value; 
       }
     ...
     var ctx= document.getElementById("chart").getContext("2d");
        ...
        var datosSet=new Array(conteo_nom_RG);
        var numFechasRecRG=new Array(conteo_fechas_rec_RG);
        var rem=0;
        
        for (i=0; i < conteo_nom_RG; i++){
        var z=0;
        var rem2=rem+conteo_fechas_rec_RG;
        for(var y=rem; y<rem2; y++){
        numFechasRecRG[z]=num_fechas_RG[y];
        z++;
        }
           
        var conteoFechasRec=0;
        for(var w=0; w<conteo_fechas_rec_RG; w++){
        conteoFechasRec++;
        }
        rem=rem+conteoFechasRec;
