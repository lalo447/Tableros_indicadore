<%-- 
    Document   : Tableros_informes_reportes_profesor
    Created on : 9 may. 2020, 9:25:30
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and u.rfc='"+rfcSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_informe, p.serie_documento, p.num_volumen, p.rango_paginas, p.fch_publicado from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and u.rfc='"+rfcSelect+"' order by u.nombre");
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           stNumRegistros=con.createStatement();
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_informe, p.serie_documento, p.num_volumen, p.rango_paginas, p.fch_publicado from usuario_profesor u, prof_colbr_infrep pr, informe_reporte p  where u.rfc=pr.rfc and pr.clave_infrep=p.clave_infrep and fch_publicado>='"+fecha_desde+"' and fch_publicado<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");
           fechasBusqueda="true";
           }            
           }
        ...
        <input type="hidden" name="asignaciones" value="Asignaciones totales de informes">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
