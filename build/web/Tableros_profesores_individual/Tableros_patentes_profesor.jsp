<%-- 
    Document   : Tableros_patentes_profesor
    Created on : 9 may. 2020, 10:53:07
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and u.rfc='"+rfcSelect+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.tipo_patente, p.pais, p.fch_presentacion, p.fch_concesion, p.resumen from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and u.rfc='"+rfcSelect+"' order by u.nombre");
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.rfc) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' group by u.rfc order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");      
           st=con.createStatement();
           rs=st.executeQuery("select u.rfc, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.estado_patente, p.tipo_patente, p.pais, p.fch_presentacion, p.fch_concesion, p.resumen from usuario_profesor u, prof_colbr_pat pr, patentes p  where u.rfc=pr.rfc and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' and u.rfc='"+rfcSelect+"' order by u.nombre");
           fechasBusqueda="true";
           }            
           }
        ...
        <input type="hidden" name="asignaciones" value="Asignaciones totales de patentes">
        <%for(int i=0; i<conteoReg2; i++){%>
        <input type="hidden" name="nombres<%=i%>" value="<%=nombres[i]%>">
        <input type="hidden" name="numRegistros<%=i%>" value="<%=numRegistros[i]%>">
        <%}%>
      ...
      <td class="text-left"><%=fecha_bus_con[i]%></td>
       <td><a href="./Resumen_tableros_profesor?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de proyecto de patente"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
       </tr>
      <%}%>
      </tbody>
      </table>
