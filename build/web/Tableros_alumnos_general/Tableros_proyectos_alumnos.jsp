<%-- 
    Document   : Tableros_proyectos_alumnos
    Created on : 13 may. 2020, 18:34:22
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
       ...
       /*Realizamos los query de consultas de proyectos*/
        ...
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.fch_inicio, p.fch_fin, p.estado_proyecto, p.tipo_actividad, p.resumen from usuario_alumno u, alm_colbr_proy pr, proyecto p  where u.numero_control=pr.numero_control and pr.clave_proyecto=p.clave_proyecto order by u.nombre");
       ...
       if(fecha_desde!=null && fecha_hasta!=null){ 
        ...
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_proy pr, proyecto p  where u.numero_control=pr.numero_control and pr.clave_proyecto=p.clave_proyecto and fch_inicio>='"+fecha_desde+"' and fch_fin<='"+fecha_hasta+"' group by u.numero_control order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_proy pr, proyecto p  where u.numero_control=pr.numero_control and pr.clave_proyecto=p.clave_proyecto and fch_inicio>='"+fecha_desde+"' and fch_fin<='"+fecha_hasta+"' order by u.nombre");
        st=con.createStatement();
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_proyecto, p.fch_inicio, p.fch_fin, p.estado_proyecto, p.tipo_actividad, p.resumen from usuario_alumno u, alm_colbr_proy pr, proyecto p  where u.numero_control=pr.numero_control and pr.clave_proyecto=p.clave_proyecto and fch_inicio>='"+fecha_desde+"' and fch_fin<='"+fecha_hasta+"' order by u.nombre");    
        fechasBusqueda="true";         
        }
        }
      ...
      <td><a href="./Resumen_tableros_alumnos?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen del proyecto"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
      </tr>
      <%}%>
      </tbody>
      </table>
