<%-- 
    Document   : Tableros_patentes_alumnos
    Created on : 14 may. 2020, 8:19:48
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        if(fecha_desde!=null && fecha_hasta!=null){ 
           ...
           rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_pat pr, patentes p  where u.numero_control=pr.numero_control and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' group by u.numero_control order by u.nombre");      
           stConteo=con.createStatement();
           rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_pat pr, patentes p  where u.numero_control=pr.numero_control and pr.clave_patente=p.clave_patente and fch_concesion>='"+fecha_desde+"' and fch_concesion<='"+fecha_hasta+"' order by u.nombre");      
           st=con.createStatement();
      ...
      <td class="text-left"><%=fecha_bus_con[i]%></td>
       <td><a href="./Resumen_tableros_alumnos?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de proyecto de patente"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
       </tr>
      <%}%>
      </tbody>
      </table>
