<%-- 
    Document   : Tableros_publicaciones_alumnos
    Created on : 13 may. 2020, 18:56:08
    Author     : laloa
--%>
...
<!--Fin de titulo horizontal-->  
	...
	<div class="content" style="z-index: 2; height: 100vh; min-width: 650px;">
        ...
        if(f2>=f1){
        stNumRegistros=con.createStatement();
        rsNumRegistros=stNumRegistros.executeQuery("select u.nombre, u.apellido_p, u.apellido_m, count(u.numero_control) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' group by u.numero_control order by u.nombre");      
        stConteo=con.createStatement();
        rsConteo=stConteo.executeQuery("select count(*) from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' order by u.nombre");      
        st=con.createStatement();
        rs=st.executeQuery("select u.numero_control, u.nombre, u.apellido_p, u.apellido_m, p.titulo_publicacion, p.tipo_publicacion, p.ano_publicacion, p.estado_publicacion, p.nombre_publicador, p.num_volumen, p.resumen, p.link_pub from usuario_alumno u, alm_colbr_pub pr, publicaciones p  where u.numero_control=pr.numero_control and pr.clave_publicacion=p.clave_publicaciones and p.ano_publicacion>='"+fecha_desde+"' and p.ano_publicacion<='"+fecha_hasta+"' order by u.nombre");
        fechasBusqueda="true";  
        }
        }
        ...
        numero_control=new String[conteoReg];
        ...
        if(conteoReg>0){
         int x=0;
        while(rs.next()){
            numero_control[x]=rs.getString(1);
      ...
      <td><a href="./Resumen_tableros_alumnos?res=<%=resumen[i]%>&titulo=<%=titulo[i]%>&tit=Resumen de la publicacion"><button class="btn resumen" style="background:#35D3C3; color:#fff; border: 1px solid #35D3C3;" title="Mostrar resumen"><i class="fas fa-file-alt"></i></button></a></td>
